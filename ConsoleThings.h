...
#endif
	char iTypeAhead[ 64];
	size_t iTypeAheadLength;
public:
	ScopedGetch();
	~ScopedGetch();
	size_t ReadBuf( char* buf, int len);
	int ReadChar();
	int ReadCmd();
...
bool DefaultFileName( char* givenFileName, char* fileNameBuffer, int bufSize, const char* type = ".wav");
char* FilenameWithChannel( char* nameBuffer, const char* baseFileName, const char* suffix, int channel, const char* type=nullptr);
char* GetFileFromList( char* fileNameBuffer, int bufLen, const char* startingFolder);
...
void DoHelp(
	const char* defaultCommand, // Command to use if argc is 0, should never need to be used
	CommandProc* cur,
	const char** cmds,
	int (** rtns)( CommandProc* cur));
